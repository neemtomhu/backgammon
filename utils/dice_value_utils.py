...
def is_move_possible(start, end, dice_value, board_state):
    # considering the board state. You might need to expand on this based on backgammon rules.
    LOG.info(f'Validating if move is possible: start={start}, end={end}, dice_value={dice_value}')
    distance = abs(start - end)
    if (distance % dice_value) != 0 or distance > (dice_value * 4):
        LOG.info('False: distance does not match dice value')
        return False
    if (board_state[start] * board_state[end]) < 0 and abs(board_state[end]) > 1:  # check if there is more than one opponent on the end field
...
def deduce_dice(detected_dice, moved_from, moved_to, board_state, turn):
    LOG.info(f'Deducing possible dice values detected_dice={detected_dice}, moved_from={moved_from}, moved_to={moved_to}')
    ...
    if len(moved_from) == 1 and total_movement != sum(detected_dice):
        if 6 < total_movement <= 12:
            ...
            possible_doubles = [(i, total_movement - i) for i in range(1, 7) if 1 <= total_movement - i <= 6]
            for dice_combination in possible_doubles:
                if all(is_move_possible(f, t, d, board_state) for f, t, d in
                       zip(moved_from, moved_to, dice_combination)):
